...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/SFDIGIHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
tags = db.Table('tags', db.Column('term_id', db.Integer, db.ForeignKey('searchterms.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')))
...
user_collection = db.Table('user_collection', db.Column('user_id', db.Integer, db.ForeignKey('personalgifcollections.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')))
...
def get_or_create_search_term(term):
    ...
    if not search:
        giphy_search = get_gifs_from_giphy(term)
        gif_collection = [(gif['title'], gif['embed_url']) for gif in giphy_search]
        search = SearchTerm(term=term)
        for item in gif_collection:
            search.gifs.append(get_or_create_gif(item[0], item[1]))
...
def create_collection():
    ...
    if request.method == 'POST':
        gifs_chosen = form.gif_picks.data
        collection_name = form.name.data
        gif_list = [get_gif_by_id(gif) for gif in gifs_chosen]
        collection = get_or_create_collection(name=collection_name, current_user=current_user, gif_list=gif_list)
        return redirect(url_for('collections', collections=collection))
